...
acquire -> work at returned path -> sync when dependencies change
        -> managed work auto-renews -> release the exact assignment ID
```
...
5. Inspect `autoRenewing`; explicitly renew only when work continues outside a
   managed Ruk operation.
...
The skill coordinates one host. It does not provide cross-host locking or
discovery of processes started outside `ruk run`. Garbage
