...
1. A preparation layer creates ordinary Git worktrees, fingerprints dependency
   ...
   [dependency implementation](../../src/dependencies.ts#L134-L181))
...
| **Managed workspace** | A pooled worktree represented by a lifecycle record (`preparing`, `assigned`, `returning`, `available`, or `failed`). ([types](../../src/types.ts#L43-L73)) |
...
| **Assignment ID / fence** | The immutable UUID that authorizes renew, process-record changes, and release. A prior ID is invalid after reuse. ([agent contract](../agent-interface.md#L34-L36), [lifecycle source](../../src/lifecycle.ts#L71-L97)) |
| **Lease** | Reporting metadata (`assignedAt`, `renewedAt`, `expiresAt`) that must be renewed explicitly. Expiry alone does not reclaim a workspace. ([types](../../src/types.ts#L45-L52), [lifecycle design](../plans/2026-08-03-workspace-lifecycle-design.md#L36-L53)) |
| **Operation ID** | A separate UUID fencing multi-step preparation and collection work while the state lock is not held continuously. ([preparation](../../src/lifecycle.ts#L100-L124), [collection](../../src/lifecycle.ts#L367-L425)) |
| **Tracked process** | A child launched through `ruk run`, recorded by PID and process-start identity (plus a POSIX process group ID) for the current assignment. ([types](../../src/types.ts#L54-L59), [run path](../../src/cli.ts#L616-L671)) |
...
`run`.
([help text](../../src/cli.ts#L47-L65),
[dispatcher](../../src/cli.ts#L681-L748))

| Command | Purpose and important behavior |
| --- | --- |
| `ruk init [--json]` / `ruk sync [--json]` | Prepare or revalidate dependencies in the current worktree. `init` and `sync` are exact dispatcher aliases. ([dispatcher](../../src/cli.ts#L696-L700), [sync](../../src/cli.ts#L154-L174)) |
| `ruk create <branch> [--path ...] [--from ...] [--detach] [--json]` | Create an ordinary worktree, prepare it, and remove it again if preparation fails. It does **not** add a pooled lifecycle/assignment record. ([source](../../src/cli.ts#L389-L437)) |
| `ruk acquire <branch> [--from ...] [--ttl ...] [--owner ...] [--json]` | Reuse the oldest available pooled workspace or create a detached, locked worktree; switch/create the requested branch, prepare dependencies, and return an assignment fence. Default TTL is 480 minutes. ([source](../../src/cli.ts#L196-L263), [contract](../agent-interface.md#L12-L36)) |
| `ruk renew <assignment-id> [--ttl ...] [--json]` | Extend only the exact active assignment; the default renewal is another 480 minutes from renewal time. ([contract](../agent-interface.md#L38-L55), [source](../../src/cli.ts#L281-L300)) |
| `ruk release <assignment-id> [--force] [--json]` | Fence the return, stop recorded children, clean the worktree, detach it, and put it back in the pool. Non-forced release preserves ownership on surviving processes or a dirty tree; forced release may kill process trees and discard all tracked, untracked, and ignored files. ([contract](../agent-interface.md#L57-L80), [source](../../src/cli.ts#L346-L386)) |
| `ruk run -- <command> [args...]` | Synchronize dependencies before execution. In an assigned pooled workspace, register the detached child against the assignment and unregister it on exit; elsewhere, simply execute it after sync. The child exit code becomes Ruk's exit code. ([source](../../src/cli.ts#L616-L671)) |
| `ruk list [--json]` | Join Git's worktree list with preparation and lifecycle state. ([source](../../src/cli.ts#L439-L469)) |
| `ruk status [--json]` | Recompute the current fingerprint, check recorded projections, and report `ready` or `sync-required` plus lifecycle/assignment fields. ([source](../../src/cli.ts#L471-L509)) |
| `ruk remove <path> [--force]` | Remove an ordinary worktree, never the current one. It refuses pooled workspaces and directs callers to `release` or `gc`. ([source](../../src/cli.ts#L512-L532)) |
| `ruk gc [--max-age ...] [--apply] [--force-expired] [--json]` | Dry-run by default. Select old `available`/`failed` records; only report expired assignments unless both `--apply` and `--force-expired` are supplied. Default age is 1,440 minutes. ([contract](../agent-interface.md#L82-L113), [source](../../src/cli.ts#L561-L613)) |
| `ruk update [--check] [--json]` | Explicitly discover a ready stable release and either report it, delegate an exact package version to the owning package manager, or replace a standalone executable. No normal command checks for updates. ([architecture](../architecture.md#L19-L46), [source](../../src/cli.ts#L735-L746)) |
...
[reporter routing](../../src/cli.ts#L138-L152))
...
1. Ruk resolves the repository root/common directory and reads `.rukrc.json`.
   ...
   ([context](../../src/cli.ts#L126-L135), [state paths](../../src/state.ts#L15-L22))
2. Under the global state lock, Ruk chooses the oldest `available` workspace
   ...
   ([reservation](../../src/lifecycle.ts#L127-L140), [acquire](../../src/cli.ts#L211-L222))
3. For a new pool member, Ruk creates a detached Git worktree and applies a Git
   ...
   ([acquire](../../src/cli.ts#L224-L241), [Git operations](../../src/git.ts#L37-L60), [assignment](../../src/git.ts#L85-L101))
4. It synchronizes dependencies inside that worktree. A newly created workspace
   ...
   ([acquire](../../src/cli.ts#L242-L260), [assignment finalizer](../../src/lifecycle.ts#L143-L157))
5. New-workspace failures become `failed` records for inspection/collection.
   ...
   ([acquire cleanup](../../src/cli.ts#L264-L278), [failure state](../../src/lifecycle.ts#L160-L179))
...
[CLI result](../../src/cli.ts#L251-L260),
...
1. Configuration accepts only `dependencyMode` and `installCommand`; unknown
   ...
   ([configuration](../../src/config.ts#L19-L67), [tests](../../test/config.test.ts#L23-L71))
...
3. Ruk acquires a per-workspace directory lock, computes the fingerprint, and
   ...
   ([source](../../src/dependencies.ts#L134-L181), [concurrency test](../../test/dependencies.test.ts#L130-L143))
4. Otherwise it performs the install. Shared Bun sets
   ...
   ([installer](../../src/dependencies.ts#L81-L119))
5. After success, Ruk recomputes the fingerprint (so install-time changes are
   ...
   ([source](../../src/dependencies.ts#L159-L174))
...
([dependency file discovery](../../src/git.ts#L146-L178),
[hash construction](../../src/fingerprint.ts#L16-L46),
...
([run path](../../src/cli.ts#L616-L662),
[runner failure handling](../../src/process.ts#L79-L145),
...
([process identity and termination](../../src/process.ts#L148-L205),
[Windows shim](../../src/process.ts#L22-L78),
...
([release implementation](../../src/cli.ts#L303-L371),
[return transitions](../../src/lifecycle.ts#L202-L251))
...
([Git return](../../src/git.ts#L103-L112),
[lifecycle finalizer](../../src/lifecycle.ts#L218-L235),
...
([collection fencing](../../src/lifecycle.ts#L367-L425),
[collection implementation](../../src/cli.ts#L534-L597),
...
- State lives at `<git-common-dir>/ruk/state.json`. Schema v2 separates
  ...
  ([types](../../src/types.ts#L75-L86), [migration](../../src/state.ts#L134-L172),
...
- Every state read is structurally validated, including absolute managed paths,
  ...
  silently replaced.
  ([validation](../../src/state.ts#L33-L173), [tests](../../test/state.test.ts#L33-L42))
- State mutation is serialized by a common directory lock, validated again,
  ...
  ([atomic update](../../src/state.ts#L189-L202), [test](../../test/state.test.ts#L8-L31))
- Locks are directories created atomically and contain mode-`0600` owner metadata
  ...
  lock.
  ([lock implementation](../../src/lock.ts#L73-L132))
- A lock older than the stale threshold is not abandoned if its owner is a live
  ...
  serialized.
  ([stale detection](../../src/lock.ts#L46-L69),
...
- The global state lock serializes short metadata mutations; a hashed
  ...
  ([lock paths](../../src/state.ts#L15-L30),
  [dependency lock](../../src/dependencies.ts#L177-L181),
  [CLI uses](../../src/cli.ts#L355-L371))
...
- Pooled worktrees are Git-locked while retained, then explicitly unlocked only
  for GC removal.
  ([lock/unlock operations](../../src/git.ts#L114-L123),
  [collection](../../src/cli.ts#L543-L551))
...
- Assignment fences apply to process-record mutation as well as renew/release,
  ...
  ([process record mutation](../../src/lifecycle.ts#L254-L308),
  [termination guard](../../src/process.ts#L175-L187))
...
[asset list](../../src/release.ts#L1-L10),
...
older ready one.
([update discovery](../../src/update.ts#L16-L18),
[ready-release selection](../../src/update.ts#L146-L181),
...
executable.
([manifest validation](../../src/update.ts#L101-L144),
[URL and download validation](../../src/update.ts#L213-L247),
[standalone update](../../src/update.ts#L351-L399))
...
([replacement](../../src/update.ts#L278-L349),
...
- The source, tests, and scripts use strict TypeScript with unchecked-index,
  ...
  [package verifier](../../scripts/verify-package.ts#L27-L44))
...
- dependency readiness is Node-ecosystem-specific: supported automatic managers
  ...
  ([detection](../../src/config.ts#L100-L118),
  [shared backend](../../src/dependencies.ts#L40-L51),
  [projection requirement](../../src/dependencies.ts#L159-L166))
- Linux arm64/musl has no standalone asset, unlike Linux x64/musl;
  ([asset selection](../../src/update.ts#L197-L210))
...
4. **What makes a pooled workspace compatible?** Selection currently takes the
   ...
   ([selection](../../src/lifecycle.ts#L127-L140),
   [acquire/sync](../../src/cli.ts#L224-L250))
5. **How far should dependency support expand?** The model is clean for
   ...
   contract would be.
   ([projection logic](../../src/dependencies.ts#L53-L73),
   [projection requirement](../../src/dependencies.ts#L159-L166))
6. **How should branch conflicts be presented?** Reassignment switches an
   ...
   agents recover.
   ([Git assignment](../../src/git.ts#L85-L101),
   [acquire cleanup](../../src/cli.ts#L264-L278))
7. **What observability is sufficient without a daemon?** JSON contracts expose
   ...
   [workspace record](../../src/types.ts#L61-L73))
8. **Should `create` and `acquire` remain separate mental models?** `create`
   ...
   ([create](../../src/cli.ts#L389-L437), [acquire](../../src/cli.ts#L196-L278),
   [remove policy](../../src/cli.ts#L512-L531))
