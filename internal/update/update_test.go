...
func TestWindowsReplacementPlanUsesFileLockWithoutPIDPolling(t *testing.T) {
	...
	for _, forbidden := range []string{"tasklist", "taskkill", "powershell", "timeout", "findstr", "4242"} {
	...
	for _, required := range []string{"copy /Y", "move /Y", "ping -n 2 127.0.0.1 >NUL", "for /f \"usebackq delims=\" %%v in (`\"C:\\bin\\ruk.exe\" --version`)", "if not \"%reportedVersion%\"==\"0.3.0-beta.1\" goto rollback", "waitAttempts=0", "waitAttempts+=1", "GEQ 120 goto wait_failed", "rollbackAttempts=0", "rollbackAttempts+=1", "if not errorlevel 1 goto rollback_succeeded", "GEQ 120 goto rollback_failed"} {
